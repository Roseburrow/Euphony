...
{
    //public RangedBandBuffer audioManager;
    ...
    private void Update()
    {
        if (audioScaling)
        {
            ...
            {
                scaleTimer += (curveScaleSpeed * RangedBandBuffer.m_rangedBounds[scaleAudioBand]) * Time.deltaTime;
                if (scaleTimer >= 1)
                {
                    scaleTimer -= 1;
                }
            ...
            {
                currentScale = Mathf.Lerp(min_maxScale.x, min_maxScale.y, RangedBandBuffer.m_rangedBounds[scaleAudioBand]);
            }
        }
        ...
        if (lerpValues)
        {
            if (isLerp)
            {
                lerpSpeed = Mathf.Lerp(min_maxSpeed.x, min_maxSpeed.y, frequencyCurve.Evaluate(RangedBandBuffer.m_rangedBounds[movevementAudioBand]));
